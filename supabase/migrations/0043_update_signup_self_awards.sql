-- Extend the player self-edit whitelist with post-event award fields so a
-- player can submit their OWN results (attended + kill/death/occupation points
-- + the wk_points total). CRITICAL: every self-write force-sets
-- awards_verified = false — a player can never self-verify, and re-editing
-- after a planner verified drops the row back to unverified for re-check.
--
-- Whitelist discipline (the march_size-0038 bug class): a field absent from
-- this SET list is silently un-editable by players, so award self-entry MUST
...
begin
  ...
  update signups set
    ...
    -- post-event self-entry (always unverified)
    attended = case when p_patch ? 'attended'
      then nullif(p_patch->>'attended','')::boolean else attended end,
    kill_points = coalesce((p_patch->>'kill_points')::bigint, kill_points),
    death_points = coalesce((p_patch->>'death_points')::bigint, death_points),
    occupation_points = coalesce((p_patch->>'occupation_points')::bigint, occupation_points),
    wk_points = case when p_patch ? 'wk_points'
      then nullif(p_patch->>'wk_points','')::bigint else wk_points end,
