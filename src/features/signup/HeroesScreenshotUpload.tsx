...
interface ExtractedHeroes {
  agent_x: number | null
  dr_j: number | null
  nataly: number | null
}

interface HeroesScreenshotUploadProps {
  onExtract: (fields: ExtractedHeroes) => void
}

const MAX_IMAGE_BYTES = 5 * 1024 * 1024

/**
 * Sibling of ProfileScreenshotUpload, but for hero-fragment screenshots.
 * Shares the same extract-profile Edge Function (different `kind: 'heroes'`
 * payload), the same rate limit (5/h per signup_token), and the same
 * UX pattern: human stays in the loop, results just pre-fill the inputs.
 *
 * Only rendered when `event.heroes_enabled` — gated by SignupPage.
 */
export function HeroesScreenshotUpload({ onExtract }: HeroesScreenshotUploadProps) {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const extracted = body.extracted as ExtractedHeroes
    ...
    <section className="rounded-lg border border-purple-500/30 bg-purple-500/5 p-3">
      <header className="mb-2 flex items-center justify-between">
        <h3 className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-purple-200">
          ...
          {t('heroes_upload.title')}
        </h3>
        <span className="text-[10px] text-zinc-400">{t('profile_upload.tag')}</span>
      </header>

      {!preview && !success && !error && (
        <p className="mb-2 text-[11px] text-zinc-400">{t('heroes_upload.description')}</p>
      )}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {busy ? t('profile_upload.detecting') : t('heroes_upload.choose_button')}
        </button>
        {preview && (
          <img
            src={preview}
